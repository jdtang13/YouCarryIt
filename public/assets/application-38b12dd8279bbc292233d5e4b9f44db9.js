...
function Cell (worldX,worldY) 
{
    ...
    this.update = function(dt)
    {
        ...
        for (var i = 0; i < this.cellWallX.length; i++) 
        {
            ...
            this.cellWallX[i] = worldX + xs + (Math.Sin(this.undulationAngle + (this.undulationAngle*i)) * undulationAmplitude);
            
			var ys = Math.sin(atEveryRadians*i) * distanceFromCenter ;
            this.cellWallY[i] = worldY + ys + (Math.Sin(this.undulationAngle + (this.undulationAngle*i)) * undulationAmplitude);
        };
    };

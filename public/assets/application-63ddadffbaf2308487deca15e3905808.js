...
var thirdStreamSentiments = [.6, .6, .7, .8, .9];
...
for (var i = 0; i < firstStreamSentiments.length; i++) {
	...
	if (firstStreamSentiments[i] > .5) {
		...
		createNutrient("glucose", (hash / Number.MAX_VALUE) * 400, (hash / Number.MAX_VALUE) * 300, 
...
for (var i = 0; i < secondStreamSentiments.length; i++) {
	...
	if (secondStreamSentiments[i] > .5) {
		...
		createNutrient("protein", (hash / Number.MAX_VALUE) * 400, (hash / Number.MAX_VALUE) * 300, 
...
for (var i = 0; i < thirdStreamSentiments.length; i++) {
	...
	if (thirdStreamSentiments[i] > .5) {
		...
		createNutrient("water", (hash / Number.MAX_VALUE) * 400, (hash / Number.MAX_VALUE) * 300, 
...
var checkCollisions = function()
{
	...
	for (var i = 0; i < freeFloatingNutrients.length; i++) 
	{
		...
		if(distanceBetweenCellAndNutrient < cellRadius + nutrientRadius)
		{
			...
			var timeOut = setTimeout(cxt.clearRect(xCor, yCor, cxt.measureText(tweet).width, 10), 500);
			clearTimeout(timeOut);

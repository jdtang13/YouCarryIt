...
function Cell (worldX,worldY) 
{
    ...
    this.update = function(dt)
    {
        ...
        {
            cellWalls[i][0] = 
            (Math.cos(Math.PI*2/cellWalls.length*i) * distanceFromCenter ;
            //     * 
            // (distanceFromCenter);//+(Math.Sin(this.undulationAngle + (Math.PI*2/cellWalls.length*i)) * undulationAmplitude));
            
            cellWalls[i][1] = 
            (Math.sin(Math.PI*2/cellWalls.length*i) * distanceFromCenter ;
            //     * 
            // (distanceFromCenter);//+(Math.Sin(this.undulationAngle + (Math.PI*2/cellWalls.length*i)) * undulationAmplitude));
